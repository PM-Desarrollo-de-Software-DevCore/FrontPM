...
test.describe('Dashboard y métricas', () => {
  test('TC-006 - Acceso a dashboard según rol', async ({ page }) => {
    await login(page, process.env.ADMIN_EMAIL || 'admin@test.com', process.env.ADMIN_PASSWORD || 'Admin123');
    ...
    await login(page, process.env.USER_EMAIL || 'user@test.com', process.env.USER_PASSWORD || 'User123');
    await page.waitForURL(/\/dashboard\/admin$/, { timeout: 10000 });
